import asyncio 
 ...
     try: 
         repo = Repo() 
         LOGGER("Ncun").info(f"Git Client Found") 
     except GitCommandError: 
         LOGGER("Ncun").info(f"Invalid Git Command") 
 ...
         LOGGER("Ncun").info("Fetched Latest Updates") 
 ...
                 LOGGER("Ncun").info(f"Heroku App Configured") 
 ...
         return 
     LOGGER("Ncun").info( 
         "TUNGGU BENTAR, LAGI MEMUAT GRUP USERBOT BUAT KAMU." 
     ) 
     desc = "Group Log buat Ubot.\n\nJangan out dari grup ini ya.\n\nPOWERED BY @chivilix"
     try: 
         gruplog = await client.create_supergroup("Ubot" , desc) 
         if await in_heroku(): 
 ...
     except Exception: 
         LOGGER("Ncun").warning( 
             "var BOTLOG_CHATID kamu belum di isi. masukan bot Rose kedalam grup kamu dan ketik /id, setelah itu isi lagi var BOTLOG_CHATID" 
         )